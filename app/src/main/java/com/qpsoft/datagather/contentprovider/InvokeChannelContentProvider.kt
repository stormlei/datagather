package com.qpsoft.datagather.contentprovider
...
class InvokeChannelContentProvider: ContentProvider()  {
    ...
    override fun insert(p0: Uri, p1: ContentValues?): Uri? {
        when(sURIMatcher.match(p0)) {
            ...
            CODE_CALLBACK_INVOKE_CHANNEL -> {
                ...
                LogUtils.i(TAG, "invokeId: $invokeId, command: $command, sourceData: $sourceData")
            ...
            CODE_NOTIFY_INVOKE_CHANNEL_EVENT -> {
                ...
                LogUtils.i(TAG, "register, eventId: $eventId, event: $event")

                mEventIdList.add(eventId)
                //notifyEvent(0, eventId, event)

                EventBus.getDefault().post(ProviderMessageEvent(eventId, event))
            }
        }
        return null
    }
    ...
    override fun call(method: String, arg: String?, extras: Bundle?): Bundle? {
        LogUtils.i(TAG, "method: $method, arg: $arg, extras: $extras")
        val bundle = Bundle()
    ...
    override fun delete(p0: Uri, p1: String?, p2: Array<String>?): Int {
        if (p2?.size == 2) {
            ...
            LogUtils.i(TAG, "unregister success, eventId: $eventId, event: $event")
        }
        return 0
    }
    ...
    private fun notifyEvent(counter: Int, eventId: String?, event: String?) {
        ...
        mHandler.postDelayed ({
            ...
            try {
                ...
                LogUtils.i(TAG, "send message success, content: event$counter success vvvvvv")
            } catch (e: Exception) {
                LogUtils.e(TAG, "callback invoke channel error")
            }
            if (counter < 10) {
